...
int infix_to_rpn(char *infix, char *postfix) {
    ...
    && infix[0] != 'a' && infix[0] != 'e' && infix[0] != 'l' && infix[0] != 'g' && infix[0] != 's' && infix[0] != 'c' && infix[0] != 't'
     && infix[0] != 'm' && infix[0] != 'n' && infix[0] != 'b' && infix[0] != 'h' && infix[0] != 'j' && infix[0] != 'k')) {
        stack_dealloc(&s);
        stack_dealloc(&functions);
        return 0;
    }
    ...
    while (infix[i] != '\0') {
        ...
        if (infix[i] == '-' && (i == 0 || infix[i - 1] == '(' || je_operator(infix[i - 1]))) {
            ...
            if (start_of_brackets_content == 1) {
                ++content_of_bracket;
            }
            postfix[j++] = ' ';
            ...
            if (content_of_bracket == 0 || je_operator(infix[i-1])) {
                stack_dealloc(&s);
                stack_dealloc(&functions);
                return 0;
            }

            content_of_bracket = 0;
